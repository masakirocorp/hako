...
Choose **Open GitHub** from Gardn's command palette. GitHub opens a built-in screen for the current client, not a managed tool tab. It uses the existing `gh` authentication. If you have not signed in, run `gh auth login` first.
