...
- **Native GitHub.** GitHub opens a built-in, client-local ratatui view inside the invoking pane rather than a terminal command or companion process. Sidebars, tabs, and other panes remain available. Switching panes or tabs preserves the view in its owner pane. Closing GitHub restores that pane's terminal. It uses the existing authenticated `gh` CLI and Gardn's palette, theme, scrollbars, and settings. There is no separate GitHub command, companion version, configuration, or theme to manage.
...
- **GitHub filters and hover.** **Queue** opens a menu of filters for the current tab, with the active filter checked. Pull requests offer Authored, Assigned, Mentioned, All, and Review requested. Issues omit Review requested. Use the mouse or arrow keys and Enter to choose; Escape cancels. Hover highlights controls and clickable rows without changing the selected item or keyboard focus.
- **GitHub actions menu.** **Actions…** opens a local menu for the current GitHub view or selected item. It does not open the global command palette. Use the mouse or arrow keys and Enter to choose an action. Escape or an outside click dismisses the menu. Long menus scroll in compact panes. Ctrl+P still opens the global command palette.
